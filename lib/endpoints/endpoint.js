...
module.exports = class Endpoint {
  ...
  async put(query = '', data = {}) {
    try {
      return await axios.put(this.endpoint + query,  querystring.stringify(data), this._headers);
    } catch (err) {
      throw new Error(`${err.response.status} error: PUT ${this.endpoint + query} -> ${this._statuses[err.response.status]}`);
    }
  }
